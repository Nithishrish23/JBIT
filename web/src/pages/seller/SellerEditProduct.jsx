...
export default function SellerEditProduct() {
  ...
  const handleChange = (e) => {
      ...
      payload.append("mrp", formData.mrp || 0);
      payload.append("stock_qty", formData.stock_qty);
      
      // Append new images
      for (let i = 0; i < newImages.length; i++) {
          payload.append("images", newImages[i]);
      }
    ...
    if (loading && !formData.name) return <div className="p-8 text-center">Loading...</div>; // Only show full loading on initial fetch
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                  ...
                  <div className="pt-6 border-t border-slate-100">
                      <label className="block text-sm font-medium text-slate-700 mb-3">Current Images</label>
                      ...
                      <label className="block text-sm font-medium text-slate-700 mb-1">Add New Images</label>
                      <input 
                          type="file" 
                          multiple 
                          accept="image/*"
                      ...
                      />
                      {newImages.length > 0 && (
                          <div className="mt-2 text-xs text-green-600">{newImages.length} new image(s) selected.</div>
                      )}
                  </div>
                  ...
                  <div className="flex gap-4 pt-4">
                      ...
                      <button
                          type="submit"
                          disabled={loading && formData.name} // Disable if saving
                          className="flex-1 px-6 py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 shadow-md hover:shadow-lg transition-all disabled:opacity-70"
                      >
                          {loading && formData.name ? 'Saving...' : 'Update Product'}
                      </button>
                  </div>
              </form>
          </div>
